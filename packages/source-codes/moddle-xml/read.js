import { forEach, find, assign } from 'min-dash'

import { Parser as SaxParser } from 'saxen'

import { Moddle, parseNameNS, coerceType, isSimpleType } from 'moddle'

import { DEFAULT_NS_MAP, getSerializationType, hasLowerCaseAlias } from './common.js'
...
Reader.prototype.fromXML = function (xml, options, done) {
  ...
  function resolveReferences() {
    ...
    for (i = 0; (r = references[i]); i++) {
      ...
      if (property.isMany) {
        ...
        // we replace an existing place holder (idx != -1) or append to the collection instead
        if (idx === -1) {
  ...
  function handleText(text, getContext) {
    // strip whitespace only nodes, i.e. before <!CDATA[ ... ]> sections and in between tags
  ...
  var uriMap = model.getPackages().reduce(
    ...
    Object.entries(DEFAULT_NS_MAP).reduce(
      function (map, [prefix, url]) {
        map[url] = prefix
